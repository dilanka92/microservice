...
import com.test.micro.modal.QuestionBuilder;
import com.test.micro.modal.Questions;

import java.util.*;
import java.util.stream.Collectors;

public class QuestionAccessibleImpl implements QuestionAccessible {

    private List<Questions> questionList;
    static private final int MAX_NUMBER_OF_QUESTIONS_PER_PAGE = 10;

    public QuestionAccessibleImpl() {
        this.loadQuestionArray();
    }

    @Override
    public Questions getQuestionByIndex(long index) {
    ...
    @Override
    public List<Questions> getQuestionList(long offset) {
        long start = offset;
        if (start < 0) {
            start = 0;
        }
    ...
    @Override
    public List<Questions> getSpecifiedQuestionList(long... id) {
    ...
    private void loadQuestionArray() {
        ...
        questionList.add((new QuestionBuilder())
                .id(0)
